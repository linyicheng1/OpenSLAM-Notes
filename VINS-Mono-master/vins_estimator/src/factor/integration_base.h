...
class IntegrationBase
{
  public:
	...
	// 预积分实际实现函数 
	// 调用中值积分函数 midPointIntegration 实现 
	// dt     时间
	// acc_1  加速度计数据
	// gyr_1  陀螺仪数据
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
		...
		// 输入：delta_q 上一次的旋转
		// 输入：delta_v 上一次的速度
		// 输入：linearized_ba 上一次的加速度
		// 输出：result_delta_p 估计得到的位置
		// 输出：result_delta_q 估计得到的旋转
		// 输出：result_delta_v 估计得到的速度
		// 输出：result_linearized_ba 估计得到的加速度
		// 输出：result_linearized_bg 估计得到的重力 
